      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
          {CERTIFICATIONS.map((cert, i) => (
            <div 
              key={i}
              className="bg-white border border-[#E1E4E3] p-6 md:p-8 rounded-lg shadow-sm hover:shadow transition duration-200 flex flex-col justify-between h-full group hover:border-[#B3401F]/35"
            >
              <div className="space-y-4">
                <div className="w-12 h-12 bg-[#B3401F]/10 rounded-xl flex items-center justify-center text-[#B3401F]">
                  <BadgeCheck className="w-6 h-6" />
                </div>
                <h3 className="font-heading font-extrabold text-lg text-[#0D1B2A] leading-snug group-hover:text-[#B3401F] transition duration-200">
                  {cert.title}
                </h3>
              ...
              <div className="pt-4 border-t border-slate-100 mt-6 flex items-center justify-between text-[10px] font-mono font-bold text-slate-400">
                ...
                <span className="text-[#B3401F] text-[11px] font-bold">{cert.authority}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white border border-[#E1E4E3] rounded-lg p-6 md:p-8 space-y-8 shadow-xs text-left">
          <div className="space-y-2 border-b border-slate-100 pb-4">
            <h3 className="font-heading font-extrabold text-lg text-[#0D1B2A] flex items-center gap-2">
              <FileCheck className="w-5 h-5 text-[#B3401F]" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 font-sans">
            {auditAgencies.map((agency, i) => (
              <div key={i} className="flex gap-3.5 items-start p-4 bg-[#F1F1ED] border border-slate-100 rounded-xl hover:bg-slate-50 transition">
                <div className="p-2 bg-white rounded-lg border border-slate-200 text-[#B3401F] shrink-0 font-mono text-xs font-bold shadow-xs">
                  0{i + 1}
                </div>
        ...
        <div className="bg-[#0D1B2A] text-white rounded-lg p-6 md:p-8 space-y-6 relative overflow-hidden border border-white/5 shadow-md">
          ...
          <div className="relative z-10 max-w-4xl space-y-4 text-left">
            <h4 className="font-heading font-extrabold text-lg text-white flex items-center gap-1.5 uppercase tracking-wide">
              <Award className="w-5 h-5 text-[#E2793D]" />
            ...
            <div className="pt-2 flex flex-wrap gap-x-8 gap-y-2 text-[10px] font-mono tracking-wider font-bold uppercase text-[#E2793D]">
